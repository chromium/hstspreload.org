...
PreloadForm.prototype = {
  ...
  showResults: function(domain, issues, status) {
    ...
    var generalElibigility = function() {
      switch (worstIssues) {
      case "none":
        this._view.setSummary("Eligibility: " + domain + " is eligible for the HSTS preload list.");
        this._view.setTheme("theme-green");
        break;
      case "warnings":
        this._view.setSummary("Eligibility: " + domain + " is eligible for preloading, although we recommend fixing the following warnings:");
        this._view.setTheme("theme-yellow");
        break;
      case "errors":
        this._view.setSummary("Eligibility: In order for " + domain + " to be elegible for preloading, the following errors must be resolved:");
    ...
    case "pending":
      this._view.setStatus("Status: " + domain + " is pending submission to the preload list.");
      this._view.setTheme("theme-green");
      break;
    ...
    case "rejected":
      if (status.message) {
        this._view.setStatus("Status: " + domain + " was previously rejected from the preload list for the following reason: " + status.message);
      } else {
        this._view.setStatus("Status: " + domain + " was previously rejected from the preload list.");
      }
      generalElibigility();
      break;

    case "removed":
      this._view.setStatus("Status: " + domain + " was previously on the preload list, but has been removed.");
      generalElibigility();
      break;
  ...
  handleResults: function(domain, status, issues) {
    ...
    if (issues.errors.length === 0 && ["unknown", "rejected", "removed"].indexOf(status.status) != -1) {
